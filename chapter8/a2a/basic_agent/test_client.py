...
async def test_basic_agent():
    ...
    async with httpx.AsyncClient() as httpx_client:
        try:
            # ① A2A 카드 리졸버 생성
            # Agent Card 정보 자동으로 가져옴 (/.well-known/agent.json)
            resolver = A2ACardResolver(
                httpx_client=httpx_client,  # async http
                base_url=base_url,
            )

            # ② 에이전트 카드 가져오기
            print("에이전트 카드를 가져오는 중...")
            agent_card = await resolver.get_agent_card() # agent metadata 가져옴 (client가 Agent의 기능 이해, 요청 )
            ...
            # Agent가 전체 응답을 생성한 후, 한 번에 Client 전송
            ...
            # 실시간 스트리밍 응답
            for i, message_text in enumerate(test_messages[:3], 1):  # 3개의 메시지만 스트리밍 테스트
                print(f"\n{i}. 사용자: {message_text}")
